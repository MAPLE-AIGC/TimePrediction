    ...
    def _initialize_weights(self):
        ...
        for m in self.fc:
            if isinstance(m, nn.Linear):
                #torch.nn.init.constant_(m.weight, 0)  # 将权重初始化为接近10
                torch.nn.init.constant_(m.bias, 2)  # 偏置可以设为0，或根据需求调整
...
class SD1_4_Text_to_Image(torch.nn.Module):
    ...
    def __init__(
        ...
        for n, p in self.time_predictor.named_parameters():
    ...
    def generate(
        ...
        if predict_next_timetsep:
            ...
            sigma = torch.sigmoid(self.time_predictor(combined))  # Shape: (bt, 1)
            #sigma = sigma.view(1)
            ...
            #next_timesteps=t-20
            return noise_pred,next_timesteps, sigma
        else:
            return noise_pred
    ...
    def forward(
        ...
        # for name, param in self.time_predictor.named_parameters():
        ...
        noise_pred,next_timestep, sigma = self.generate(
