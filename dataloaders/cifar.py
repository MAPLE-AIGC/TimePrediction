...
def collate_fn(batch):
    images = torch.stack([torch.from_numpy(item[0]).float() / 255 * 2 - 1 for item in batch], dim=0)
...
def load_cifar(
    ...
    dataset = ImageFolderDataset(
        path, resolution=32, use_labels=True)
    sampler = InfiniteSampler(
        dataset, rank=dist.get_rank(), num_replicas=dist.get_world_size())
