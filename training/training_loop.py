...
def validation_step(
    ...
    for prompt in prompts:
        ...
        while t_cur > epsilon:
            timesteps.append(t_cur.item())
            noise_pred,next_timestep, sigma = net.generate(
                latents=latents,
                t=t_cur,
    ...
    while True:
        ...
        for round_idx in range(num_accumulation_rounds):
            with misc.ddp_sync(ddp_net, (round_idx == num_accumulation_rounds - 1)):
                ...
                mse_loss, sigma_loss, total_loss= return_dict['mse_loss'], return_dict['sigma_loss'], return_dict['total_loss']
                training_stats.report('Loss/mse_loss', mse_loss)
                training_stats.report('Loss/sigma_loss', sigma_loss)
                training_stats.report('Loss/total_loss', total_loss)

                total_loss.backward()
            losses.append(total_loss.detach().mean())
        
        if dist.get_rank() == 0:
            logger.logger.add_scalar('Loss/mse_loss', mse_loss.detach().cpu().item(), global_step=training_step)
            logger.logger.add_scalar('Loss/sigma_loss', sigma_loss.detach().cpu().item(), global_step=training_step)
            logger.logger.add_scalar('Loss/total_loss', total_loss.detach().cpu().item(), global_step=training_step)
        # loss logging begin
        #sigmas = torch.concat(sigmas, dim=0)
        losses = [loss.unsqueeze(0) if loss.dim() == 0 else loss for loss in losses]
        ...
        if cur_tick % val_ticks == 0 and dist.get_rank() == 0:
            ...
            with torch.autocast(device_type="cuda", enabled=True, dtype=precision_dtype):
                images = validation_step(
                    ...
                    prompts=[
                        ...
                        "A young girl with blonde hair is sitting on the floor", 
                        "A modern brick building with a mix of black and white windows", 
                        "A man with short hair wearing a dark blazer and a patterned scarf",
                        'A small cactus in the desert',
                        'A serene beach scene with a clear sky and a few clouds',
                        "A calico cat with black patches is lying on a grey and white patterned blanket",
                        "A white and black dog is wrapped in a white towel, lying on a bed with a floral pattern",
                        "A bumblebee is perched on a lavender flower in a field",
                        "A quaint stone building with a brown roof is surrounded by a lush garden filled with vibrant flowers"
                    ],
            ...
            for i, image in enumerate(images):
                image = image.cpu().detach().float()
                logger.logger.add_image(f"Images/{i}", image, global_step=training_step)
        # Update state.
        cur_tick += 1
        cur_nclip = 0
        tick_start_step = training_step
